...
CPP_GCC4_FLAGS = -fvisibility-inlines-hidden -Wno-deprecated-register
CPP = clang
...
CPPFLAGS += -Wno-non-virtual-dtor -fno-rtti -fno-threadsafe-statics -Wno-overloaded-virtual -Wno-uninitialized -Wno-delete-non-virtual-dtor -std=c++11
...
all:
	ln -sf  $(HL2LIB)/libvstdlib$(LIBPOSTFIX).so libvstdlib$(LIBPOSTFIX).so;
	ln -sf  $(HL2LIB)/libtier0$(LIBPOSTFIX).so libtier0$(LIBPOSTFIX).so;
